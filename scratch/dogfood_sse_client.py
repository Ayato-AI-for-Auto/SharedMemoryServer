        ...
        async with sse_client(server_url) as streams:
            async with ClientSession(streams[0], streams[1]) as session:
                        ...
                        "entities": [
                            {
                                ...
                                "entity_type": "concept",
                                "description": (
                                    "An entity created during automated SSE dogfooding test."
                                ),
                            }
                        ],
                        "observations": [
                            {
                                "entity": "DogfoodingNode",
                                "content": "This node was created by an automated client test.",
                            }
                        ],
                    },
                )
                ...
                await session.call_tool(
                    "manage_knowledge_activation", {"ids": ["DogfoodingNode"], "status": "inactive"}
                )
                ...
                await session.call_tool(
                    "manage_knowledge_activation", {"ids": ["DogfoodingNode"], "status": "active"}
                )
