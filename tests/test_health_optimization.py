...
from shared_memory.database import init_db, get_connection
...
async def test_comprehensive_diagnostics(mock_gemini):
    ...
    with patch("shared_memory.health.check_disk_usage", return_value={
        "dir": "/tmp", "total": 10**12, "used": 0, "free": 10**12, "percent_free": 100.0
    }):
        # Mock Gemini to be healthy
        mock_gemini.models.list.return_value = [type('Model', (), {'name': 'models/gemini-pro'})]
...
async def test_bulk_embedding_optimization(mock_gemini):
    ...
    mock_gemini.models.embed_content.return_value = MockResp([MockEmb(mock_vecs[0]), MockEmb(mock_vecs[1])])
