...
def test_update_access_and_stability(temp_db):
    ...
    conn = get_connection()
    conn.execute("INSERT INTO entities (name, entity_type) VALUES ('test_node', 'test')")
    conn.commit()
    
    # First access
    update_access("test_node")
    
    row = conn.execute("SELECT access_count, stability FROM knowledge_metadata WHERE content_id = 'test_node'").fetchone()
    assert row[0] == 1
