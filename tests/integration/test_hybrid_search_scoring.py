...
@pytest.mark.asyncio
async def test_hybrid_search_scoring_integration(mock_llm):
    """
    Integration test: Verify that hybrid search weights keyword and
    semantic results correctly.
    """
    # 1. Setup Data
...
async def test_search_no_results_integration(mock_llm):
    """
    Verify behavior when no matches are found in either
