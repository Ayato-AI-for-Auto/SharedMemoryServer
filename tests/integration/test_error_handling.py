...
from shared_memory.database import async_get_connection, DatabaseLockedError
...
async def test_integration_handles_locked_db(mock_llm):
    ...
    with patch("aiosqlite.Connection.execute", side_effect=sqlite3.OperationalError("database is locked")):
...
async def test_integration_path_traversal_prevention():
    ...
    bank_files = [
        {"filename": "../unauthorized.txt", "content": "I should not be here"}
    ]
