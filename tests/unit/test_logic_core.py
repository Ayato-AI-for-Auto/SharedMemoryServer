...
async def test_save_memory_entities_unit(fake_llm):
    ...
    async with await async_get_connection() as conn:
        cursor = await conn.execute("SELECT name, entity_type FROM entities WHERE name = ?", ("UnitEntity",))
        row = await cursor.fetchone()
        assert row is not None
        assert row[1] == "test"
        
        # Verify Audit Log
        cursor = await conn.execute("SELECT action FROM audit_logs WHERE table_name = 'entities' AND content_id = ?", ("UnitEntity",))
...
async def test_save_memory_relations_unit(fake_llm):
    ...
    relations = [
        {"subject": "UnitA", "object": "UnitB", "predicate": "links_to"}
    ]
...
async def test_save_memory_observations_unit(fake_llm):
    ...
    fake_llm.models.set_response(
        "generate_content", 
        json.dumps({"conflict": True, "reason": "Scientific contradiction"})
    )
...
async def test_save_memory_invalid_data_unit(fake_llm):
    ...
    res = await save_memory_core(relations=[
