...
async def test_background_init_success(fake_llm):
    ...
    with patch("shared_memory.server.init_db", new_callable=AsyncMock) as mock_db, \
         patch("shared_memory.server.thought_logic.init_thoughts_db", new_callable=AsyncMock) as mock_thought:
...
async def test_background_init_failure(fake_llm):
    ...
    with patch("shared_memory.server.init_db", side_effect=Exception("DB Crash")), \
